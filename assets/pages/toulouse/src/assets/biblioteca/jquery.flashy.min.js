  ...
  (c.prototype = {
    ...
    _buildDOM: function () {
      (this.controls.$overlay = l("<div></div>").addClass("flashy-overlay")),
        (this.controls.$container = l("<div></div>")
          .addClass("flashy-container")
          .attr("tabindex", 1)),
        (this.controls.$title = l("<div></div>").addClass("flashy-title")),
        (this.controls.$numeration = l("<div></div>").addClass(
          "flashy-numeration"
        )),
        ...
        (this.controls.$preloader = l("<div></div>").addClass(
          "flashy-preloader"
        )),
        this.controls.$overlay.append(
          this.controls.$container,
          this.controls.$title,
          this.controls.$numeration,
          this.controls.$next,
          this.controls.$prev,
          this.controls.$close,
          this.controls.$preloader
        ),
        this.$body.append(this.controls.$overlay),
        this.$body.get(0).offsetHeight;
      var t =
        this.controls.$container.get(0).offsetWidth -
        this.controls.$container.get(0).clientWidth;
      this.controls.$container.css("right", -t + "px"),
        this.$body.addClass("flashy-active");
    },
    _bind: function (t) {
      t.config.overlayClose
        ? this.controls.$overlay.on(
            "click touchend",
            l.proxy(this._onOverlayClick, this)
          )
        : this.controls.$close.on(
            "click touchend",
            l.proxy(this._onCloseClick, this)
          ),
        this.controls.$prev.on(
          "click touchend",
          l.proxy(this._onGalleryNavigationClick, this, "prev")
        ),
        this.controls.$next.on(
          "click touchend",
          l.proxy(this._onGalleryNavigationClick, this, "next")
        ),
        this.$body.on("keydown", l.proxy(this._onKeyboard, this)),
        t.config.gallery &&
          ("ontouchstart" in e
            ? t.config.gallerySwipeMobile &&
              (this.$document.on("touchstart", l.proxy(this._onTouch, this)),
              this.$document.on("touchmove", l.proxy(this._onTouch, this)),
              this.$document.on("touchend", l.proxy(this._onTouch, this)))
        ...
        this.$document.on(
          ...
          l.proxy(this._onFullsceenChange, this)
        );
    },
    _unbind: function () {
      this.controls.$overlay.off(
        "click touchend",
        l.proxy(this._onOverlayClick, this)
      ),
        this.controls.$close.off(
          "click touchend",
          l.proxy(this._onCloseClick, this)
        ),
        this.controls.$prev.off(
          "click touchend",
          l.proxy(this._onGalleryNavigationClick, this, "prev")
        ),
        this.controls.$next.off(
          "click touchend",
          l.proxy(this._onGalleryNavigationClick, this, "next")
        ),
        this.$body.off("keydown", l.proxy(this._onKeyboard, this)),
        "ontouchstart" in e
          ? (this.$document.off("touchstart", l.proxy(this._onTouch, this)),
            this.$document.off("touchmove", l.proxy(this._onTouch, this)),
            this.$document.off("touchend", l.proxy(this._onTouch, this)))
    ...
    _onOverlayClick: function (t) {
      l(t.target).closest(".flashy-content").hasClass("flashy-content") ||
        this._close();
    },
    ...
    _onKeyboard: function (t) {
      null != this.currItem &&
        ...
        (27 !== t.keyCode
          ? this.currItem.config.gallery &&
            (37 == t.keyCode && this.prevItem && this._galleryNavigate("prev"),
            39 == t.keyCode && this.nextItem && this._galleryNavigate("next"))
          : this._close());
    },
    ...
    _onTouch: function (t) {
      ...
      (o =
        e == this.touchMouseEvent.up
          ...
          : this._normalizeEvent(e, t, n.pageX, n.pageY)),
        l(t.target).trigger(o);
    },
    ...
    _onSwipeMove: function (t) {
      if (this.swipe.startouch) {
        ...
        (10 <= e || 10 <= o) &&
          (t.preventDefault(),
          this.controls.$content.css(
            "transform",
            "translate(" + this.swipe.diffX + "px," + this.swipe.diffY + "px)"
          ));
      }
    },
    _onSwipeUp: function (t) {
      if (this.swipe.startouch) {
        ...
        (this.swipe.diff = this.swipe.endX - this.swipe.startX),
          ...
          o && Math.abs(this.swipe.diff) >= this.swipe.threshold
            ? this._galleryNavigate(e)
    ...
    _show: function (t, e) {
      var o = t.data(a),
        ...
        l = this;
      null == this.currItem &&
        (this._buildDOM(), this._initGalleryCounter(n), this._bind(n)),
        ...
        this._loadContent(s, n, i)
          ...
          .fail(function () {
            l.controls.$content
              .html(n.config.loadError)
              .addClass("flashy-error"),
              l._showContent(n, e);
          })
          .always(function () {
    ...
    _initSwipe: function (t) {
      (this.swipe.threshold = t.config.gallerySwipeThreshold),
        this.controls.$contentInner.on(
          this.touchMouseEvent.down,
          l.proxy(this._onSwipeDown, this)
        ),
        this.controls.$contentInner.on(
          this.touchMouseEvent.move,
          l.proxy(this._onSwipeMove, this)
        ),
        this.controls.$contentInner.on(
          this.touchMouseEvent.up,
          l.proxy(this._onSwipeUp, this)
        );
    },
    _hideContent: function (t, e) {
      if (this.controls.$contentOuter) {
        ...
        "next" == e && t.config.nextHideClass
          ? (n = t.config.nextHideClass)
          : "prev" == e &&
            t.config.prevHideClass &&
            (n = t.config.prevHideClass),
    ...
    _showContent: function (t, e) {
      ...
      "next" == e && t.config.nextShowClass
        ? (o = t.config.nextShowClass)
        : "prev" == e && t.config.prevShowClass && (o = t.config.prevShowClass),
    ...
    _loadContent: function (t, e, o) {
      this._showPreloader(),
        ...
        (this.controls.$contentOuter = l("<div></div>").addClass(
          "flashy-content-outer"
        )),
        (this.controls.$contentInner = l("<div></div>").addClass(
    ...
    _loadVideo: function (t, e, o) {
      ...
      var n = this._parseVideoUrl(o),
        s = null,
        i =
          (e.config.videoAutoPlay ? "?rel=0&autoplay=1" : "?rel=0") +
          this._getUrlParameter(o);
      "vimeo" == n.type
        ? (s = "https://player.vimeo.com/video/")
        : "youtube" == n.type && (s = "https://www.youtube.com/embed/"),
        s
          ? (this.controls.$content.html(
              '<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen frameborder="0" src="' +
                s +
                n.id +
                i +
                '"></iframe>'
    ...
    _parseVideoUrl: function (t) {
      ...
      return (
        ...
        -1 < RegExp.$3.indexOf("youtu")
          ? (e = "youtube")
          : -1 < RegExp.$3.indexOf("vimeo") && (e = "vimeo"),
        { type: e, id: RegExp.$6 }
      );
    },
    ...
    _galleryNavigate: function (t) {
      null != this.currItem &&
        ("next" == t && this.nextItem
          ? this._show(this.nextItem.$el, t)
          : "prev" == t && this.prevItem && this._show(this.prevItem.$el, t));
    },
    ...
    _updateGalleryNavigation: function (t) {
      if (t.config.gallery) {
        ...
        for (o = 0; o < e; o++)
          if (
            this.items.storage.hasOwnProperty(o) &&
            (n = this.items.storage[o]).group == t.group
          ) {
            s = n;
            break;
          }
        ...
        s && i && s.id == i.id
          ? this._hideGalleryNavigation()
          : (t.config.galleryLoop &&
              (null == l && (l = s), null == r && (r = i)),
            l && t.config.galleryPrevNext
              ? this.controls.$next.addClass("flashy-show")
              : this.controls.$next.removeClass("flashy-show"),
            r && t.config.galleryPrevNext
    ...
    _initGalleryCounter: function (t) {
      var e = 0,
        ...
        s = null;
      for (s = 0; s < n; s++)
        this.items.storage.hasOwnProperty(s) &&
          this.items.storage[s].group == t.group &&
          e++;
      for (s = 0; s < n; s++)
        this.items.storage.hasOwnProperty(s) &&
          this.items.storage[s].group == t.group &&
          ((this.items.storage[s].groupIndex = ++o),
          (this.items.storage[s].groupTotal = e));
    },
    _updateGalleryCounter: function (t) {
      if (
        t.config.gallery &&
        t.config.galleryCounter &&
        t.config.galleryCounterMessage &&
        1 < t.groupTotal
      ) {
        var e = t.config.galleryCounterMessage
          .replace("[index]", t.groupIndex)
          .replace("[total]", t.groupTotal);
    ...
    _updateTitle: function (t) {
      if (t.config.title) {
        ...
        e
          ? this.controls.$title.addClass("flashy-show").html(e)
    ...
    _updateDOM: function (t) {
      this._updateTitle(t),
        ...
        t.config.themeClass &&
          this.controls.$overlay.addClass(t.config.themeClass);
    },
    show: function (t) {
      this._show(t);
    },
